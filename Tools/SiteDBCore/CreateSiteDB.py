...
class CreateSQLiteDB(CreateSiteDBHelper):
  # FIXME: connection should really happen somewhere else.
  def __init__(self, schema=None, path=None, dbname=None, verbose=None, test=None, connection=None):
    if not connection:
      from pysqlite2 import dbapi2 as sqlite 
      if verbose:
        print "Creating database in path = " + path + dbname
      
        con = sqlite.connect(os.path.join (path, dbname))
    else:
      con = connection
    ...
    if verbose:
      print "SQLite database exists: %s - database creation completed" % os.path.exists(os.path.join (path, dbname))
  ...
  **/
  ...
  def initTables(self, con=None, path=None, dbname=None, verbose=None, test=None):
    if con != None:
      ...
      INSERT_CONTACT1 = '''insert into contact ( surname, forename, email, dn, phone1, username)
          values('admin', 'admin', 'hn-cms-webInterfaces@cern.ch', 'n=admin', '0123456789', 'admin')'''
      ...
      INSERT_PASSWORD1 = ''' insert into user_passwd (username, passwd)
        values('admin', \'''' + crypt.crypt ("admin", "fo") + '''\')'''
      ...
      INSERT_RESPONSIBILITY8 = '''insert into group_responsibility(contact, user_group, role) values(1, 1, 1)'''
      #INSERT_RESPONSIBILITY9 = '''insert into group_responsibility(contact, user_group, role) values(1, 1, 1)'''
