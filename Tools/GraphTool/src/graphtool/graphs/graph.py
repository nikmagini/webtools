...
class Grapher( Cache, QueryHandler ):
    ...
    def do_multiprocess(self, graph_instance, results, file, metadata, **kw):
        q = multiprocessing.Queue()
        p = multiprocessing.Process(target=self._do_multiprocess_child,
            args=(q, graph_instance, results, file, metadata), kwargs=kw,
            name="GraphTool Graph Generator")
        p.daemon = True
        p.start()
        graph_results, file_val = q.get()
        p.join()
        file.write(file_val)
