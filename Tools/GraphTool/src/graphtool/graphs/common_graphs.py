...
class BarGraph( PivotGraph ):
    ...
    def get_coords( self ):
        ...
        for pivot in keys:
            bar = coords[pivot]
            t = bar.get_transform()
            my_coords = t.transform( bar.get_verts() )
  ...
  def get_coords( self ):
    ...
    for pivot in keys:
      ...
      for group, p in groupings.items():
        t = p.get_transform()
        my_coords = t.transform( p.get_verts() )
        height = self.prefs['height']
  ...
  def get_coords( self ):
    ...
    for idx in range(len(timebins)-1):
      ...
      for pivot in keys:
        ...
        my_coords = transform.transform( [(time_begin, bottom_left), (time_begin, csum_left), \
...
class PieGraph( PivotGraph ):
  ...
  def setup( self ):
    ...
    for key in keys:
      labels.append( str(key) + (' (%s)' % pretty_float(round(float(parsed_data[key])))) )
      amt.append( float(parsed_data[key]) )
    self.labels = labels
    self.labels.reverse()
    self.title = title % pretty_float(int(float(sum(amt))))
...
class QualityMap( HorizontalGraph, TimeGraph, PivotGroupGraph ):
  ...
  def setup( self ):
    ...
    # Determine the lineskip, i.e. the distance between two horizontal dotted lines
    self.lineskip = self.metadata.get('lineskip', 2)
    
    # Determine if we should draw the border around data rectangles
    self.rectangleborder = self.metadata.get('rectangleborder', True)
